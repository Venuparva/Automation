...
def runTest(params):
   ...
   Account_configure_result = Outlookclient.Actions.LoginintoOutlook.ConfigureAccountinOutlook(userName, password)
   if "Success" in Account_configure_result:
      logNote("Report: OutlookClient - Account Configuration result : pass")
   else:
      logNote("Report: OutlookClient - Account Configuration result : fail")
      upload_video()
