...
def runTest(params):
    global Account_configure_result, orig_subject, Account_remove_result, Send_mail_result, Folder_selection_result, Search_Mail_result, from_addr, to_addr, cc_addr, bcc_addr, subject, folder, body_of_mail, attachment_file_path, userName, password, searchHeader, searchText, Check_delivery_result, Delete_msgs_result, Delete_folder_result, draft_subject, Copy_msgs_result, cleanupFolder, imap_list_cmd,Clean_up_account_result,draft_sub_time,existing_count,Reply_mail_result
    ...
    if "attachment" in str(params):
   	...
   	# -------Reply mail---------
    Reply_mail_result = Outlookclient.Actions.ReplyMail.reply_mail(from_addr, to_addr,subj,body,attachment_file_path)
    if "Success" in Reply_mail_result:
        logNote("\nReport : Outlookclient - Reply_mail_result : pass")
   	else:
        logNote("\nReport : Outlookclient - Reply_mail_result: fail")