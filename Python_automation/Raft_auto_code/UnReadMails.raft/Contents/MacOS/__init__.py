...
def runTest(params):
    global userName, password, newFolder,expected_output,trash_flag,imap_list_cmd,Change_environ_result,environ,Account_configure_result,Account_remove_result,flag_folder,read_folder,read_count
    ...
    if "folder" in str(params):
        ...
        logNote("User provided folder text:", str(read_folder))
    else:
        read_folder = "Inbox"
    if "Count" in str(params):
        Count = params['Count']
        read_count = Count
    # -----------------MARK as UNREAD mails from between default Folders---------------
    UnRead_msgs_result = Outlookclient.Actions.MarkAsUnread.markMailasUnRead(read_folder, read_count)
    logNote("---------------------------")
    if "Success" in UnRead_msgs_result:
        ...
        if "Success" in str(server_validation_result):  # since FLAGGED word should not be there after unflag operation
            logNote("\nOutlookclient - UNREAD mails: server-side validation result : %s" % (str(server_validation_result)))
            logNote("\nReport : Outlookclient -UnRead_msgs_result : pass")
        else:
            logNote("\nOutlookclient - UNREAD mails: server-side validation status : %s" % (str(server_validation_result)))
            logNote("\nReport : Outlookclient -UnRead_msgs_result : fail")
    else:
        logNote("\nReport : Outlookclient -UnRead_msgs_result: fail")