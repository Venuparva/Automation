...
userName = password = newFolder = expected_output = trash_flag = imap_list_cmd = Account_configure_result = Create_folder_result = Account_remove_result = Change_environ_result = flag_folder = flag_count = Flag_msgs_result = ''' '''
...
def runTest(params):
    global userName, password, newFolder,expected_output,trash_flag,imap_list_cmd,Change_environ_result,environ,Account_configure_result,Account_remove_result,flag_folder,flag_count,Flag_msgs_result
    ...
    if "folder" in str(params):
        flag_folder = params['folder']
        logNote("User provided folder text:", str(flag_folder))
    else:
        flag_folder = "Inbox"
    if "Count" in str(params):
        Count = params['Count']
        flag_count = Count
    # -----------------FLAG mails from in Folders---------------
    logNote("---------------------------")
    Flag_msgs_result = Outlookclient.Actions.FlagMail.flagMails(flag_folder, flag_count)
    logNote("---------------------------")
    if "Success" in Flag_msgs_result:
        ...
        if "Success" in str(server_validation_result):
            logNote("\nReport: server-side validation result : %s" % (str(server_validation_result)))
            logNote("\nReport : Outlookclient -Flag_msgs_result : pass")
        else:
            logNote("\nReport: server-side validation status : %s" % (str(server_validation_result)))
            logNote("\nReport : Outlookclient -Flag_msgs_result : fail")
    else:
        logNote("\nReport : Outlookclient - Flag_msgs_result: fail")